    ...
    <div className="flex h-dvh w-full overflow-hidden">
      <main className="relative flex min-w-0 flex-1 flex-col">
        ...
        <header className="safe-px relative z-10 flex h-8 shrink-0 items-center gap-2 border-b-2 border-ink/80 bg-cream">
          {/* Sous 420 px de large, le bandeau réclamait 430 px et le bouton
              « Journal » sortait de l'écran, sans rien pour le signaler. La
              marque est ce qui se retire le mieux : elle ne sert qu'à revenir à
              l'accueil, ce que le geste de retour du navigateur fait déjà. */}
          <Link
            href="/"
          ...
          <section className="flex shrink-0 items-start gap-3" aria-label="Adversaires">
            {opponents.map((p) => (
              <OpponentSeat
                ...
                seatWidth={Math.floor(
                  (rowWidth - 12 * (opponents.length - 1)) / opponents.length,
                )}
